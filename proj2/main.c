...
int generate_workload(Process *workload, int seed) {
  ...
  while (!valid) {
    for (int i = 0; i < count; i++) {
      ...
      workload[i].priority = rand() % 4; // ✅ FIXED
    }
...
// Export Gantt CSV
void export_gantt_csv(const char *algo_name, int run_id,
                      const char *time_chart) {
  char filename[64];
...
void run_simulation_step(const char *name, int run_id, AlgoFunc func,
                         Process *workload, int count, SimulationStats *stats,
                         bool export_csv) {
  Process p[MAX_JOBS];
...
int main(int argc, char *argv[]) {
  ...
  AlgoFunc funcs[] = {
      run_FCFS,          run_SJF, run_SRT, run_RR, run_HPF_NonPreemptive,
      run_HPF_Preemptive};
  ...
  for (int run = 0; run < NUM_RUNS; run++) {
    ...
    for (int i = 0; i < 6; i++) {
      run_simulation_step(names[i], run, funcs[i], workload, count, &stats[i],
                          enable_csv);
    }
  }

  printf("\n[==========] Final Statistics (Average over 5 runs)\n");
  printf("%-10s %-12s %-12s %-12s %-12s\n", "Algorithm", "Avg TAT", "Avg Wait",
         "Avg Resp", "Throughput");
